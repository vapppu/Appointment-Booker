...
def session_calendars():
    ...
    if "logged_in" in session:
        ...
        calendars = conn.execute(
            'SELECT id FROM calendar WHERE owner_id = ?', (session["user"],)).fetchall()
        calendar_list = []
...
def print_appointments(rows):
    ...
    for dict in appointments_dict:
        dict["string"] = format_timestring(
            dict["starting_time"], dict["ending_time"])
...
def print_available(calendar_id):
    ...
    appointments = conn.execute(
        "SELECT * FROM appointment WHERE calendar_id = (?) AND available = 0 ORDER BY starting_time", (calendar_id, )).fetchall()
    conn.close()

    appointments_dict = [{k: item[k] for k in item.keys()}
    ...
    if request.method == "POST":
        ...
        if "user" in session:
            ...
            user_data = conn.execute(
                "SELECT * FROM user WHERE id = (?)", (session["user"],)).fetchone()
            calendar_list = []
            for calendar in session["calendars"]:
                data = conn.execute(
                    "SELECT * FROM calendar WHERE id = (?)", (calendar, )).fetchone()
...
def register():
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        conn.execute("INSERT INTO user (username, firstname, lastname, email, password_hash) VALUES (?,?,?,?,?)",
...
def login():
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        data = conn.execute(
            'SELECT * FROM user WHERE username = ?', (username,)).fetchall()
        conn.close()
...
def new_calendar():
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO calendar (name, owner_id, additional_info) VALUES (?,?,?)" "RETURNING id",
                       (title, user_id, additional_info))
...
def appointments(calendar_id):
    ...
    conn = get_connection()
    calendar_data = conn.execute(
        "SELECT * FROM calendar WHERE id = (?)", (calendar_id,)).fetchone()
    appointments_dict = []
    conn.close()
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        appointments = conn.execute(
            "SELECT * FROM appointment WHERE calendar_id = (?)", (calendar_id, )).fetchall()
        conn.close()
        ...
        if valid_appointment_list(appointments, start, end, None):
            ...
            conn = get_connection()
            conn.execute(
                "INSERT INTO appointment (calendar_id, starting_time, ending_time, available) VALUES (?,?,?,?)", (calendar_id, start, end, 0))
            conn.commit()
            conn.close()
        ...
        appointment_data = conn.execute(
            "SELECT * FROM appointment LEFT OUTER JOIN answer ON appointment.id = answer.appointment_id WHERE calendar_id = (?) ORDER BY appointment.starting_time", (calendar_id, ))
...
def edit_calendar(calendar_id):
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        conn.execute("UPDATE calendar SET name = (?), additional_info = (?) WHERE id = (?);",
                     (newtitle, new_info, calendar_id))
        conn.commit()
        conn.close()
...
def delete_calendar(calendar_id):
    ...
    conn.execute(
        "DELETE FROM answer WHERE appointment_id IN (SELECT id FROM appointment WHERE calendar_id = (?))", (calendar_id, ))
    conn.execute(
        "DELETE FROM appointment WHERE calendar_id = (?)", (calendar_id, ))
...
def attend(calendar_id):
    ...
    if calendar_data != None:
        user_data = conn.execute(
            "SELECT id, firstname, lastname, email FROM user WHERE id = (?)", (calendar_data["owner_id"], )).fetchone()
        conn.close()
    else:
        conn.close()
        return render_template("notfound.html")
    ...
    if request.method == "POST":
        ...
        conn = get_connection()
        data = conn.execute(
            "SELECT available FROM appointment WHERE id = (?)", (appointment, )).fetchone()
        available = data["available"]

        # If appointment is available, insert data into answer table in database.
        if available == 0:
            conn.execute(
                "UPDATE appointment SET available = 1 WHERE id = (?)", (appointment, ))
            conn.execute(
                "INSERT INTO answer (appointment_id, timestamp, participant_name, participant_email) VALUES (?,CURRENT_TIMESTAMP,?, ?)", (appointment, name, email))
            conn.commit()
            conn.close()
...
def edit_appointment(calendar_id, appointment_id):
    ...
    if request.method == "POST":
        ...
        new_starting_time = datetime.fromisoformat(
            request.form.get("starting_time"))
        new_ending_time = datetime.fromisoformat(
            request.form.get("ending_time"))
        ...
        if valid_appointment_list(appointments, new_starting_time, new_ending_time, index):
            ...
            conn = get_connection()
            conn.execute("UPDATE appointment SET starting_time = (?), ending_time = (?) WHERE id = (?)",
                         (new_starting_time, new_ending_time, appointment_id))
            conn.commit()
            conn.close
